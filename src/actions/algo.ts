...
export function findStationsByName(
  ...
  return graph
    .nodes()
    .filter((node) => graph.getNodeAttribute(node, "name") === stationName);
}
 ...
 */
export function computeFairNode(
  graph: Graph,
  sourceIds: string[]
): [string | null, number] {
  ...
  for (const src of sourceIds) {
    ...
    for (const node in paths) {
      try {
        distances[src][node] = calculatePathLength(
          graph,
          paths[node],
          "weight"
        );
 ...
 */
export function computeFairForStations(
  graph: Graph,
  stationNames: string[]
 ...
 * @param stationNames Array of station names.
 */
export function displayTravelTimesFromCenter(
  graph: Graph,
  centerNode: string,
  stationNames: string[]
): void {
